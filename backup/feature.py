...
def computeHarrisValues(srcImage):
        ...
        # Sobel operators to compute x and y derivatives
        Ix = ndimage.sobel(srcImage, axis=0, mode='nearest')
        Iy = ndimage.sobel(srcImage, axis=1, mode='nearest')
        
        # Elements of the Harris matrix H
        Ix2 = ndimage.gaussian_filter(Ix**2, sigma=0.5)
        Iy2 = ndimage.gaussian_filter(Iy**2, sigma=0.5)
        Ixy = ndimage.gaussian_filter(Ix*Iy, sigma=0.5)
...
def detectCorners(harrisImage, orientationImage):
        ...
        # Compute local maxima in the Harris response
        localMaxima = computeLocalMaximaHelper(harrisImage)
        
        # Select points that are local maxima and have a strong Harris response
        features = np.argwhere(localMaxima & (harrisImage > np.percentile(harrisImage, 90)))  # Choosing top 10% strong corners
